...
export const getStudentByName = (fullName) => {
    return students.find(student => {
        ...
        if (studentName.toUpperCase() === fullName.toUpperCase()) return student
    })
}
...
export const getInstructorOfStudent = (studentId) => {
    return instructors.find(instructor => {
        const chosenStudent = students.find(student => student.id === studentId)
        if (instructor.id === chosenStudent.instructorId) return instructor
    })
}
...
export const getStudentWithMostLangs = () => {
    ...
    for (student of students) {
        if (student.languages.length > mostLang) {
            mostLang = student.languages.length
            studentMost = student
        }
    }
    return studentMost
