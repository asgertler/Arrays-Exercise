...
export const getAvailableInstructors = () => {
    return instructors.filter(instructor => {
        const isTeaching = students.find(student => student.instructorId === instructor.id)
...
export const getStudentsByLanguage = (language) => {
    return students.filter(student => {
        return student.languages.some(languageSpoken => languageSpoken === language)
    })
}
...
export const getStudentsByLanguages = ([languages]) => {
    return students.filter(student => {
        if (student.languages === languages) return student
    })
}